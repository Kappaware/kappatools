...
public class ConfigurationImpl implements Configuration {
	...
	public ConfigurationImpl(ParametersImpl parameters) throws ConfigurationException, PropertyVetoException {
		...
		if (this.parameters.getJdbcDriverClassName() != null) {
			jdbcDriverClassName = this.parameters.getJdbcDriverClassName();
		} else {
			String jdbcUrl = this.parameters.getJdbcUrl();
			String[] splits = jdbcUrl.split(":");
			if (splits.length < 3) {
				throw new ConfigurationException(String.format("Invalid jdbcUrl:'%s'", jdbcUrl));
			}
			DbType dbType;
			try {
				dbType = DbType.valueOf(splits[1]);
			} catch (IllegalArgumentException e) {
				throw new ConfigurationException(String.format("Invalid database type:'%s'", splits[1]));
			}
			switch (dbType) {
				case phoenix:
		...
		this.targetDataSource = new ComboPooledDataSource();
		this.targetDataSource.setDriverClass(jdbcDriverClassName);
