...
public class ParametersImpl implements Parameters {
	...
	static OptionSpec<String> NODES_OPT = parser.accepts("nodes", "Cassandra nodes").withRequiredArg().describedAs("node1[,node2,...]").ofType(String.class).required();
	static OptionSpec<Integer> PORT_OPT = parser.accepts("port", "Cassandra access port").withRequiredArg().describedAs("port (Default: 9042)").ofType(Integer.class);
	static OptionSpec<String> TARGET_TABLE_OPT = parser.accepts("table", "target table").withRequiredArg().describedAs("keySpace.table").ofType(String.class).required();
	...
	public ParametersImpl(String[] argv) throws ConfigurationException {
		try {
			...
			this.nodes = result.valueOf(NODES_OPT);
			this.port = result.valueOf(PORT_OPT);
